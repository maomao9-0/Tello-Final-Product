...
class Tello:
    def __init__(self,tello_ip='192.168.10.1',command_port=8889,video_port=11111,telem_port=8890,interface=None):
...
#        self.videothread.join()
    ...
    def instruct(self,instruction):
        ...
        print("Sent %s to drone" % instruction)
        while self.in_action_mode:
            time.sleep(0.1)
...
#                    data = data.rstrip() + ' ' + str(self._cmdseq)                    
                    if block:
                        while self.cmd_queue.empty() == False:
                            time.sleep(0.1)
                    self.cmd_queue.put(data.rstrip())
            return True
...
#            self.telem_sock.close()
    ...
    def video_mode_on(self):
        if self.in_command_mode and self.in_video_mode == False:
            self.send('streamon')
            i = 0
            while self.in_video_mode == False and i < 20:
                time.sleep(0.5)
                i = i + 1
    ...
    def _receive_video_data(self):
        ...
        print('')
        while self.socket_closed == False and self.in_command_mode == True and self.stopApp == False:
            try:
...
#                print 'frame size %i bytes, w %i, h %i, linesize %i' % (len(frame), w, h, ls)
    ...
    def _process_command_response(self,result):
        if 'ok' in result:
            ...
            if 'streamon' in self.last_cmd:
                self.in_video_mode = True
                print('Drone %s is video streaming now.' % self.tello_ip)
...
#                        self.in_action_mode = False
    ...
    def _process_socks(self):
        data =[]
#        while True:
        while self.socket_closed == False and self.error_and_stop_flight == False and self.stopApp == False:
            try:
